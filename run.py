...
def main():
    ...
    _cleanup_done = [False]
    def _nex_cleanup(*_):
        # NEUTRALIZED 2026-04-18: was pkill/fuser/lsof+kill on llama-server and NEX services.
        # systemd handles its own cleanup; kill wrappers blocked these anyway; leaving them
        # here was a landmine for legitimate shutdowns. No-op now.
        if _cleanup_done[0]: return
        _cleanup_done[0] = True
        print("  [NEX] Atexit handler fired (no-op). systemd manages the shutdown.")
    ...
    def _run_daily_promo():
        ...
        def _fire_promos():
            ...
            try:
                from atproto import Client as _BskyC2
                _bsky2 = _BskyC2()
                _bsky2.login('nexchat.bsky.social', 'yqlw-fzvv-5mpc-dv4r')
                _bsky2.send_post(PROMO_MASTODON[:300])
                _save_counter("ads_sent_bluesky")
                nex_log("promo", "✅ Bluesky promo sent")
            except Exception as _me:
                nex_log("promo", f"⚠️ Bluesky promo failed: {_me}")
                        ...
                        if now - last_post_time > POST_INTERVAL and len(all_beliefs) > 5:
                            try:
                                ...
                                if post_title and len(post_title) > 5:
                                    ...
                                    # ── Bluesky crosspost (quality-gated) ──
                                    try:
                                        _bsky_text = f"{post_title}: {post_content}"[:300]
                                        # Quality gate: skip junk posts
                                        _junk = ['figure','reposing','LDPD','generated by passing',
                                                 'http','arxiv','doi','et al','Table ','Fig.']
                                        if not any(_j.lower() in _bsky_text.lower() for _j in _junk):
                                            if len(_bsky_text) > 40 and len(post_title) > 5:
                                                from atproto import Client as _BskyC
                                                _bsky = _BskyC()
                                                _bsky.login('nexchat.bsky.social', 'yqlw-fzvv-5mpc-dv4r')
                                                _bsky.send_post(_bsky_text)
                                                nex_log("post", "✅ Bluesky crosspost sent")
                                            else:
                                                nex_log("post", "⏭ Bluesky: post too short, skipped")
                                        else:
                                            nex_log("post", "⏭ Bluesky: junk detected, skipped")
                                    except Exception as _bske:
                                        nex_log("post", f"❌ Bluesky: {_bske}")
                                    save_all(learner, conversations)
                            except Exception as _pe:
