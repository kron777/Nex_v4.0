...
class Layer4_Synthesis:
    ...
    def _assemble_position(self, top_beliefs: list,
        ...
        noise = [
            ...
            'different domain', 'domain a', 'domain b',
            '↔', 'synthesized around', 'sounds like a different',
        ]
...
class PredictionErrorLoop:
    ...
    def compute_error(self, query: str,
        ...
        for topic in all_topics:
            p = predicted.get(topic) or 0.0
            a = actual.get(topic) or 0.0
            surprise[topic] = abs(a - p)

        total_surprise = sum(v for v in surprise.values() if v is not None)
        self.errors.append({
            'query':     query[:50],
...
class NexBeliefReservoirEngine:
    ...
    def process(self, query: str, topics: list = None) -> dict:
        ...
        # FIX A: broadened query finds tensions even with sparse edges
        ...
        result.update({
            ...
            'new_gaps':  new_gaps,
            'surprise':  sum(v for v in surprise.values() if v is not None),
...
class NativeVoice:
    ...
    OPENERS = [
        ...
        "I've been sitting with the idea that",
        "Here's what I can't get past:",
        "Something shifts when I consider that",
        "The thing this keeps coming back to is",
        "I hold this as genuinely unresolved:",
        "What changes when I sit with this:",
    ]
    _used_openers: list = []
    ...
    def speak(self, synthesis: dict) -> str:
        if not synthesis.get('position') or synthesis.get('needs_llm'):
            ...
            _lg.getLogger('nex.nbre').debug(
                f"[NativeVoice.speak] skipped — position={bool(synthesis.get('position'))} "
                f"needs_llm={synthesis.get('needs_llm')} n_fired={synthesis.get('n_fired',0)}"
            )
            return None
        ...
        import random
        # Cooldown: avoid repeating last 4 openers
        _available = [o for o in self.OPENERS
                      if o not in NativeVoice._used_openers[-4:]]
        if not _available:
            _available = self.OPENERS
        opener = random.choice(_available)
        NativeVoice._used_openers.append(opener)
        if len(NativeVoice._used_openers) > 16:
            NativeVoice._used_openers = NativeVoice._used_openers[-8:]
