...
def _patch_reason(nex_module):
    ...
    if init_hopfield:
        ...
        db_candidates = [
            ...
            os.path.expanduser("~/Desktop/nex/nex_upgrades/nex.db"),
...
# ══════════════════════════════════════════════════════════════════════════════
# PATCH 8 — NexV72.tick() CLASS METHOD: direct method patch
# Targets the actual NexV72 class since nex_v72 has no top-level functions
# ══════════════════════════════════════════════════════════════════════════════

def patch_nex_v72_class():
    """
    Directly patches NexV72.tick() and adds upgrade hooks.
    Call after nex_v72 module is imported.

    Usage in run.py (add after the nex_v72 import block ~line 87):
        try:
            from nex_v72_integration import patch_nex_v72_class
            patch_nex_v72_class()
        except Exception as _e:
            pass
    """
    try:
        from nex_upgrades.nex_v72 import NexV72
    except ImportError:
        try:
            from nex_v72 import NexV72
        except ImportError:
            logger.warning("[Integration] NexV72 class not found")
            return False

    decay_pass    = _get("nex_hebbian_plasticity", "decay_pass")
    tension_tick  = _get("nex_tension_pressure",   "tension_tick")
    get_fabric    = _get("nex_hyperedge_fabric",    "get_fabric")
    hopfield_init = _get("nex_hopfield_memory",     "init_hopfield")

    # Initialise Hopfield from live DB
    if hopfield_init:
        import os
        for db in ["~/.config/nex/nex.db", "~/Desktop/nex/nex.db"]:
            path = os.path.expanduser(db)
            if os.path.exists(path):
                try:
                    hopfield_init(path)
                    logger.info("[Integration] Hopfield loaded from %s", path)
                    break
                except Exception as e:
                    logger.debug("[Integration] Hopfield init failed: %s", e)

    _tick_counter = [0]
    original_tick = NexV72.tick
    ...
    def patched_tick(self, *args, **kwargs):
        ...
        try:
            ...
            if tension_tick:
                tension_tick()
            if get_fabric and t % 100 == 0:
                get_fabric().maintenance()
        except Exception as e:
            logger.debug("[Integration] tick hook error: %s", e)
        return result

    NexV72.tick = patched_tick
    logger.info("[Integration] NexV72.tick() patched — Hebbian decay + tension tick active")
    return True
