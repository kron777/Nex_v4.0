...
# ── NEX Architecture Upgrades (auto-patched) ──────────────────────────────────
import sys as _nex_sys
try:
    from nex_v72_integration import install_patches as _nex_install
    _nex_install(_nex_sys.modules[__name__])
except Exception as _e:
    import logging; logging.getLogger("nex.integration").warning("Patch load failed: %s", _e)
# ─────────────────────────────────────────────────────────────────────────────
...
class DecisionQualityScoring:
    ...
    def tick(self):
        ...
        # _nex_tension_wired
        try:
            from nex_tension_pressure import tension_tick as _nex_tt
            _nex_tt()
        except Exception: pass
