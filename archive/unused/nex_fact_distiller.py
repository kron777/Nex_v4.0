...
_fh = logging.FileHandler(os.path.join(_LOG_DIR, "fact_distiller.log"))
...
EXTRACTION_PROMPT = """You are a fact extractor. Read the article below and extract specific, verifiable factual claims.
...
def _extract_rss_links(xml_text: str, max_links: int = 3) -> list[str]:
    """Pull article URLs from RSS/Atom feed XML."""
...
def _is_feed(raw_text: str) -> bool:
    """Detect if fetched content is RSS/Atom XML."""
...
def distil_url(url: str, topic_hint: str = "") -> int:
    ...
    If the URL is an RSS/Atom feed, follows article links.
    ...
    # If it's a feed, extract article links and distil those
    if _is_feed(raw):
        links = _extract_rss_links(raw, max_links=3)
        if not links:
            log.debug(f"{LOG} feed had no article links: {url}")
            return 0
        total = 0
        for link in links:
            try:
                text = _fetch_text(link)
                facts = extract_facts_from_text(text, link, topic_hint)
                ...
                log.debug(f"{LOG} article fetch failed {link}: {e}")
        if total:
            print(f"{LOG} +{total} facts from feed {url[:60]}")
        return total

    # Regular page — strip HTML and extract
    text = re.sub(r"<[^>]+>", " ", raw)
