...
def _nbre_preload():
    ...
    def _load():
        try:
            ...
            _eng.load()
            _live = _sys2.modules.get('nex_soul_loop') or _sys2.modules.get('__main__')
            if _live:
    ...
    def respond(self, query: str) -> str:
        ...
        try:
            ...
            if getattr(_nsl, '_nbre_singleton', None) and getattr(_nsl, '_nbre_ready', False):
                ...
                print(f"[NBRE] fired={_nr.get('n_fired',0)} conf={_nr.get('confidence',0):.2f} "
                      f"needs_llm={_nr.get('needs_llm',True)} rate={_nr.get('llm_rate',0):.1%} "
                      f"tensions={len(_nr.get('tensions',[]))}")
